...
<!--HEADER-->
	...
	<div class="content">
		<div class="mar1000 clearfix">
			...
			<div class="rights">
				<div class="path path_nomar">
					<label>促销活动</label><span>当前位置：首页 &gt; 促销活动</span>
				</div>
			<c:forEach var="packlist" items="${packageList}">
			<div class="news_item">
            	<a href="PackageDetailServlet?id=${packlist.id}"><img src="${packlist.img}" width="180" height="122"  alt=""/></a>
            	<div class="news_info">
                	<h2><span>${packlist.time}</span><a href="PackageDetailServlet?id=${packlist.id}">${packlist.title}</a></h2>
                </div>
            </div>
			</c:forEach>
            <div class="page">一共${requestScope.total}页
            <a href="PackageListServlet?current=1">首页</a>
            <a href="PackageListServlet?current=${current-1}">上一页</a>
            <a href="PackageListServlet?current=${current+1}">下一页</a>
            <a href="PackageListServlet?current=${total}">尾页</a><br>
