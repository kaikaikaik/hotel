 ...
 */
@WebServlet("/AddSaleListServlet")
public class AddSaleListServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		try {
			...
			if(flag==1){
				request.getRequestDispatcher("addsalelist.jsp").forward(request, response);
			}
			java.util.Date date = null;
			date = new SimpleDateFormat("yyyy-MM-dd").parse(time);
			java.sql.Date newdate = new java.sql.Date(date.getTime());
			his.AddSaleList(title, from1, info, newFileName, newdate);
